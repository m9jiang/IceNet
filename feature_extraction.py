...
from models import SSResNet_Encoder  # noqa: E402
...
def main(root: str, config_path: str, model_path: str) -> None:
    ...
    for idx, dir_name in enumerate(dirs):
        ...
        feature_map[0, :, :] = hh
        feature_map[1, :, :] = hv
        model = SSResNet_Encoder.ResNetEncoder(config['model'])
        ...
        map_img.save(os.path.join(root, dir_name, f'ResNet_patch_{patch_size}.png'))
        map_color_img = Image.fromarray(map_color)
        map_color_img.save(os.path.join(root, dir_name, f'ResNet_patch_{patch_size}_color.png'))
